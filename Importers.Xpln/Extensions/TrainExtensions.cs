...
internal static class XplnTrainExtensions
{
    extension(Train train)
    {
        ...
        public (Maybe<StationCall> call, int index) FindBetweenArrivalAndDeparture(string stationSignature, Time time, SheetRow sheetRow)
        {
            if (train.TryFindCall(stationSignature, sheetRow, (c) => true, out (Maybe<StationCall> call, int index) result1))
                return result1;
            if (train.TryFindCall(stationSignature, sheetRow, (c) => c.Arrival == time, out (Maybe<StationCall> call, int index) result4))
                return result4;
            else if (train.TryFindCall(stationSignature, sheetRow, (c) => c.Departure == time, out (Maybe<StationCall> call, int index) result5))
            {
                return result5;
            }
            if (train.TryFindCall(stationSignature, sheetRow, (c) => time >= train.Calls.Last().Arrival && c.Equals(train.Calls.Last()), out (Maybe<StationCall> call, int index) result2))
                return result2;
            if (train.TryFindCall(stationSignature, sheetRow, (c) => time <= train.Calls.First().Departure && c.Equals(train.Calls.First()), out (Maybe<StationCall> call, int index) result3))
                return result3;
            else
            {
                train.TryFindCall(stationSignature, sheetRow, (c) => time > c.Arrival && time < c.Departure, out (Maybe<StationCall> call, int index) result6);
                return result6;
            }
        }
        private bool TryFindCall(string stationSignature, SheetRow sheetRow, Func<StationCall, bool> compare, out (Maybe<StationCall> call, int index) result)
        {
            ...
            {
                result = (new Maybe<StationCall>(string.Format(CultureInfo.CurrentCulture, Resources.Strings.TrainHasNoCallsAtStation, sheetRow, train, stationSignature)), -1);
                return false;
            }
            else
            {
                result = (new Maybe<StationCall>(string.Format(CultureInfo.CurrentCulture, Resources.Strings.TrainHasOverlappingTimesAtStation, sheetRow, train, stationSignature)), -1);
                return false;
            }
        }
    }
