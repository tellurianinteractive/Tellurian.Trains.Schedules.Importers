...
**train parts** in working order. Use **Build automatically** to chain trains of the same category
...
one by hand. On a row, **+ train** appends the next train — choose only part of it (a from/to stop)
when a train must be split between vehicles, for example at a change from electric to diesel
traction. **+ vehicle** assigns a vehicle, creating a new one when needed; a schedule may carry
