        ...
        if (!timetableStretchId.HasValue)
        {
            ...
            if (timetableStretchId.HasValue)
            {
                ...
                foreach (var stretch in timetableStretch.Stretches)
                {
                    ...
                    if (stretchId.HasValue)
                    {
                        AccessRepository.ExecuteNonQuery(connection, TimetableTrackStretches.CreateInsertCommand(timetableStretchId.Value, stretchId.Value, sequenceNumber));
                    }
                }
                return true;
            }
