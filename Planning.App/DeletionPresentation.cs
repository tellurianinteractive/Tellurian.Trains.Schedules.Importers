...
public static class DeletionPresentation
{
    ...
    /// <summary>
    /// A short, localised explanation of why an object cannot be deleted: the <paramref name="leadKey"/>
    /// message followed by up to <see cref="MaxListed"/> of the referencing items and "(and N more)" when
    /// there are further ones — so the user can find and remove the references. Returns <c>null</c> when
    /// <paramref name="deletion"/> is not a failure, so a bound <c>title</c> attribute is simply omitted.
    /// </summary>
